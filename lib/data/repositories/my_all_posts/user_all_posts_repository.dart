...
class UserAllDisasterPostRepository extends GetxController {
  static UserAllDisasterPostRepository get instance => Get.find();
  ...
  Future<List<DisasterModel>> getDisasterDetails() async {
    try {
      ...
      final snapShot = await _db.collection('Disasters').where('userId', isEqualTo: currentUser.uid).limit(10).get();

      final disasterList = snapShot.docs.map((e) => _mapToDisasterModel(e)).toList();
      print('Fetched ${disasterList.length} disaster(s) for user: ${currentUser.uid}');
      return disasterList;
